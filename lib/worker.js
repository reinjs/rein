...
module.exports = class Worker extends Koa {
   ...
   */
  async initialize() {
    await this._app.initialize(['controller', 'middleware', 'service'], false, originalPrototypes);
    this._inited = true;
  }
   ...
   */
  async listen() {
    if (!this._inited) await this._app.initialize(['controller', 'middleware', 'service'], false, originalPrototypes);
    ...
    const server = await new Promise((resolve, reject) => {
      ...
      this._app.invoke('starting', serve).then(() => {
    ...
    await this._app.invoke('started', server);
