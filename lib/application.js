...
module.exports = class Application {
  ...
  error(err, code) {
    err = new Error(err);
    err.code = err.status = !isNaN(code) ? Number(code) : 500;
    return err;
  }
   ...
   */
  starting(fn) {
    return this._pushLifeCycle('starting', fn);
  }
        ...
        case 'message':
          this.loader.addMessageCompiler(this.app, ({ type, pathname }) => {
            if (type === 'project') return {
              file: path.resolve(pathname, `app/${this.app.name}.js`),
              name: type
            };
            return {
              file: path.resolve(pathname, 'app/message.js'),
              name: type
            };
          });
          break;
