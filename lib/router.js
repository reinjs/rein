...
module.exports = class KoaRouter extends Router {
  ...
  _socket(name, path) {
    ...
    super.register(path, ['SOCKET'], middleware, { name });
    return this;
  }
  
  socket(...args) {
    const length = args.length;
    return this._socket(...args.map((arg, index) => {
      if (index === 0 && isRule(arg)){
        this.sockets.push(arg);
        return arg;
      }
      if (isExtra(arg, index, length)) return this.extraTransfer(arg, args[0], 'SOCKET');
      if (index === length - 1) return this.socketTransfer(arg);
  ...
  socketTransfer(arg) {
    return async (ctx, next) => {
      const target = this.controllerTransfer(arg);
      if (!target) return await next();
      ctx.res.socketResult = target;
      ctx.res.socketContext = ctx;
      ctx.status = 200;
    }
  }
  ...
  functional(object, arg) {
    return arg.split('.').reduce((target, property) => {
      ...
      throw new Error('[koa-router transfer] can not find property of ' + property + ' on `' + arg + '`');
    }, object);
  }
